...
my $search = '^S7L:([\d -:\.]+).+p('.$parser.'[0-9abcdef ]+)$' if ($timestamp == 1);
$search = '^.+p('.$parser.'[0-9abcdef ]+)$' if ($timestamp == 0);

#print "Search: $search\n";
# Opening septel.log file from working directory
open INIFILE, "< $inifile" or die "Can't open INI file : $!";
close INIFILE;
open INPUTFILE, "< $workdiri\\$inputfile" or die "Can't open Input file : $!";
